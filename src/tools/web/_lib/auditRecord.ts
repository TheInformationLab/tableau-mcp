...
export const auditRecordSchema = z.object({
  ...
  target: z.object({
    ...
    // 'user' is used by update-user / delete-user (site membership mutations). 'group' is used by
    // delete-group. Both are retained in the enum so parsers stay stable across additions.
    kind: z.enum(['datasource', 'workbook', 'extract-refresh-task', 'user', 'group']),
  }),
